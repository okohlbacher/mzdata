...
impl<'a> FrameToArraysMapper<'a> {
    ...
    pub fn process_3d_slice(&self, iv: impl RangeBounds<usize>) -> BinaryArrayMap3D {
        // BRFP vendor patch (mobiusklein/mzdata#50): `scan_offsets` is a cumulative
        // prefix-sum of peak OFFSETS into tof_indices/intensities, length
        // `n_scan_rows + 1` (last entry = total peak count). Scan `k`'s peaks are
        // tof_indices[scan_offsets[k] .. scan_offsets[k + 1]].
        //
        // The previous loop confused scan indices with these offsets: it only worked
        // when first_scan == 0 (MS1 / first window) and otherwise prepended every
        // earlier scan's peaks, stamped with one wrong mobility, inflating MS2 peak
        // counts ~1.4-2.0x. It also labelled each scan's peaks with the *next* scan's
        // mobility, and its end-bound arithmetic was inverted. This rewrite indexes
        // scans directly: each scan is read exactly once with its own mobility.
